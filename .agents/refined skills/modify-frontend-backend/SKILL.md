...
description: Make a coordinated change across the frontend and backend so the two halves stay in lockstep across a contract surface (REST, WebSocket, node shape, session shape, compose, modulators). Use when the user says 'Modify Frontend + Backend', or asks for a change that spans both sides. For the general, app-agnostic version of this pattern see coordinated-change.
---

# Modify Frontend + Backend

**Trigger:** `Modify Frontend + Backend` (also fires on "keep the API and UI in sync", "change both sides of the contract").
...
This skill exists because agents tend to update the JSX, declare "done," and leave the backend behind. The workflow here forces a contract-first approach: identify what crosses the boundary, then update both sides together.
...
- For waveform changes: read `backend/tests/test_waveform_parity.py` — this test fails loudly if parity breaks, and is the canonical proof of correctness.
...
- **Version control:** no auto-PR. Leave commits to the user unless they ask for the Jules `submit` flow.
- **Generalizing:** this skill encodes this app's specific contract surfaces. For the reusable contract-first pattern that underlies it, see `coordinated-change`.
