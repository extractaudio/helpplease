...
const displayTime = (value: string) => { const [hourText, minute] = value.split(':'); const hour = Number(hourText); const suffix = hour >= 12 ? 'PM' : 'AM'; const displayHour = hour % 12 || 12; return `${displayHour}${minute === '00' ? '' : `:${minute}`} ${suffix}` }
...
export function storeHoursSummary(status: StoreOpenStatus, instant: Date) {
  ...
  const parts = local(instant); const time = `${parts.hour}:${parts.minute}`
  if (time < status.opensAt) return `Opens today at ${displayTime(status.opensAt)}`
  const tomorrow = local(new Date(instant.getTime() + 24 * 60 * 60 * 1000)); const next = hoursForDay(tomorrow.weekday)
